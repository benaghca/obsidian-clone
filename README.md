...
- **Status bar**: backlinks and the note's open tasks (click either to see them), the selection's word count, the cursor's line and column, words and reading time (click for characters), and the editing mode (click to switch between reading and editing, or right-click for source mode).
- **Focus mode** (**Ctrl+Alt+Z**, the ◎ in the status bar, or Settings → Editor) hides the side bars and tabs and dims everything but the paragraph you're writing. **Typewriter scrolling** keeps the line you're typing on in the middle of the window.
