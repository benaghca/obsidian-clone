...
- **Embedded web pages**: `![](https://example.com)` on its own line shows the live page in a sandboxed frame, in live preview and reading view. `![|600x400](…)` sets the size, and YouTube and Vimeo links become their players. The bar above the page reloads it or opens it in your browser. Canvas link cards show the live page too (click a card to use its page). **Settings** can load pages on click instead, or show just the link.
